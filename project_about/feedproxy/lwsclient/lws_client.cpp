...
static int LwsClientCb(struct lws *wsi, enum lws_callback_reasons reason, void *user, void *in, size_t len)
{
    ...
    switch (reason)
    {
        ...
        case LWS_CALLBACK_WSI_DESTROY:
            isNeedRecon = true;
            std::cout<<"Lws start reconnect"<<std::endl;
            break;
        default:
            break;
    }
...
int LwsClient::Connect(std::string addr, int port)
{
    ...
    protocols.rx_buffer_size = 5 * 1024 * 1024; // 一次回调接收的数据大小10MB,服务器回包的大小不能超过10M，否则手机服务器将会直接断开连接。
    protocols.id = 0;
    protocols.user = NULL;
